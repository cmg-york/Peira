...
# https://extras.springer.com/?query=978-3-030-73479-4
# Remove everything from k3.R except bootstrap.ci
source("k3.R")


#
# Adapting the bootstrap routine to our needs
#
bootstrap.ci.stu <- function(x, conf.level, R){
  b_res = bootstrap.ci(x, conf.level, R)
  result = c("lower" = b_res[1],"mean" = mean(x),"upper" = b_res[2])
  return (result)
}
...
getData <- function(languag,dataPath,keyPath) {
  ...
  # Analysis does not filter anything with PILOT 
  #  because authoritative have changed.
  d0 = d %>%
  #  filter(!participant %in% d.retest.ids$parti) %>%
    ...
    pivot_longer(cols = -participant,
                 names_to = "item",
                 values_to = "response") %>% left_join(kkt,by = "item")
  ...
  if ("PROLIFIC_PID" %in% colnames(d)){
    clean = d0 %>% filter(itMatters) %>% 
      group_by(participant) %>% summarise(wrong = mean(response!=authoritative)*n(),total = n()) %>%
  ...
  #
  # Identify retest information 
  #
  if ("PROLIFIC_PID" %in% colnames(d)){
    # Get all the data incl. test and retest
    d.retest.basis = d %>% 
      select(participant, PROLIFIC_PID, TIME_end) %>% distinct()
    # Extract those that are later and are of IDs that have been seen before
    d.retest.ids = d.retest.basis %>% 
      # Get the clean data
      filter(participant %in% clean$participant) %>%
      group_by(PROLIFIC_PID) %>% summarise(retest.time = max(TIME_end),
                                           maintest.time = min(TIME_end)) %>%
      # Remove those who did not do retest
      filter(retest.time!= maintest.time) %>% 
      # Keep track of times and participant ids.
      inner_join(d.retest.basis %>% rename("retest.id" = "participant"), 
                 by = c("PROLIFIC_PID","retest.time" = "TIME_end") )  %>%
      inner_join(d.retest.basis %>% rename("maintest.id" = "participant"), 
                 by = c("PROLIFIC_PID","maintest.time" = "TIME_end") )
  } else {
    d.retest.ids = tribble(~PROLIFIC_PID,~retest.time,~maintest.time,~retest.id,~maintest.id)
  }
  
  
  # d1: observational data (main) -------------
  d1 = d %>% 
    # Exclude the retest entries
    filter(!participant %in% (d.retest.ids %>% pull(retest.id))) %>% 
    # Clean
    inner_join(clean, by = "participant") %>%
    select(participant,starts_with(languag)) %>% 
    pivot_longer(cols = starts_with(languag),
                 names_to = "psy_item",
                 values_to = "response")  %>% right_join(kk,by = "psy_item") 
  ...
  # d2: attitudinal data (main) -----------------------
  d2 = d %>%
    # Exclude the retest entries
    filter(!participant %in% (d.retest.ids %>% pull(retest.id))) %>% 
    # Clean
    inner_join(clean,by = "participant") %>%
    select(participant, starts_with("concept"),starts_with("relationship"),
           -starts_with("conceptU"),-starts_with("relationshipU")) %>%
    pivot_longer(cols = c(starts_with("concept"),starts_with("relationship")),
                 names_to = "psy_item",
                 values_to = "value") %>% left_join(kka, by = "psy_item")
  
  
  # d2.retest: attitudinal data (retest) ----------------
  # It has to be the prolific version
  if ("PROLIFIC_PID" %in% colnames(d)){
    d2.retest = d %>%
      # Exclude the retest entries
  ...
  de = 
    rbind(
      ...
      ,
      d %>% 
        select(participant,"Sex:1",starts_with("Fl:"),starts_with("english")) %>%
        # Exclude the retest entries
        filter(!participant %in% (d.retest.ids %>% pull(retest.id))) %>% 
        pivot_longer(cols = c("Sex:1",starts_with("Fl:"),starts_with("english")),
                     names_to = "item",
                     values_to = "response") %>% inner_join(read_csv(paste0(keyPath,"demos.csv")),by = c("item", "response")) %>%
        select(participant,demo_item_name,demo_item_label)
    )  %>% inner_join(clean,by = "participant")
  
  
  if ("PROLIFIC_PID" %in% colnames(d)){
    cl = list(clean = d %>% select(participant,PROLIFIC_PID) %>% inner_join(clean,by = "participant"),
              unclean = d %>% select(participant,PROLIFIC_PID) %>% anti_join(clean,by = "participant"))
  } else {
    cl = NULL
  }
  ...
  return(list(obs = d1, 
              ...
              demo = de,
              clean = cl,
              retest = list(obs.retest = d1.retest,
                            self.retest = d2.retest,
                            key = d.retest.ids)))
}



getObservationalData <-  function(lan,dataPath,configPath){
  #lan = "iRL"
  #dataPath = "Data/config - 25-02-24 22-45-35/"
  #configPath = "Data/config - 25-02-24 22-45-35/"
  d = read_csv(paste0(dataPath,"data.csv"))
  key = read_csv(paste0(configPath,"key.csv"))
  
  # d1: observational data (main) -------------
  d_long = d %>% 
    select(participant,starts_with(lan)) %>% 
    pivot_longer(cols = starts_with(lan),
                 names_to = "psy_item",
                 values_to = "response")  
  
  d1 = d_long %>% right_join(key,by = "psy_item") 
  
  return(d1)
}

getAttitudinalData <-  function(lan,dataPath,configPath){
#  lan = "iRL"
#  dataPath = "Data/config - 25-02-24 22-45-35/"
#  configPath = "Data/config - 25-02-24 22-45-35/"
  d = read_csv(paste0(dataPath,"data.csv"))
  key_att <- tibble(read.csv(paste0(configPath,"key-attitudinal.csv"))) %>% filter(language == lan)
  
  d2_long = d %>%
    select(participant, starts_with("concept"),starts_with("entity"),starts_with("relationship"),
           -starts_with("entityU"),-starts_with("relationshipU")) %>%
    pivot_longer(cols = c(starts_with("concept"),starts_with("entity"),starts_with("relationship")),
                 names_to = "psy_item",
                 values_to = "value") 
  
  d2 = d2_long %>% left_join(key_att, by = "psy_item")
}

getDemographicData <- function(lan,dataPath,configPath){
  
  #lan = "iRL"
  #dataPath = "Data/config - 25-02-26 12-58-16/"
  #configPath = "Data/config - 25-02-26 12-58-16/"
  d = read_csv(paste0(dataPath,"data.csv"))
  demos = read_csv(paste0(configPath,"demos.csv"))
  
  # de: demographic data ---------------
  de = 
    rbind(
      d %>% 
        select(participant,"Age_1") %>% rename(demo_item_label = "Age_1") %>% mutate(demo_item_name = "Age") 
      ,
      d %>% 
        select(participant,"Sex_1",starts_with("Fl_"),starts_with("english")) %>%
        pivot_longer(cols = c("Sex_1",starts_with("Fl_"),starts_with("english")),
                     names_to = "item",
                     values_to = "response") %>% 
        inner_join(demos,by = c("item", "response")) %>%
        select(participant,demo_item_name,demo_item_label)
    ) 
    return(de)
}
...
accuracyBoot <- function(df, conceptStruct, conf = 0.9, iter = 999) {
  #df = acc.data
  #conceptStruct = list(iRL = getConceptCodeList_Entities("iRL",concepts_df))
  #conf = 0.9
  #iter = 999
  #l = "iRL"
  ...
  allone <- function(v){
    return( (length(unique(v)) == 1) & (v[1] == 1) | (length(v) == 0))
  }
  ...
  for (l in names(conceptStruct)) {
    for (c in conceptStruct[[l]]) {
      ...
      if (!allzero(base$acc) & !allone(base$acc)) {
        accCI = bootstrap.ci.stu(base$acc,conf.level = conf, R = iter) # Bootstrap over all accuracy values.
      } else if (allzero(base$acc)) {
        accCI = c(0,0,0)
      } else {
        accCI = c(1,1,1)
      }
      ...
      if (!allzero(base$def) & !allone(base$def)) {
        defCI = bootstrap.ci.stu(base$def,conf.level = conf, R = iter) # Bootstrap over all deficiency values.
      } else if (allzero(base$def)) {
        defCI = c(0,0,0)
      } else {
        defCI = c(1,1,1)
      }
      
      if (!allzero(base$exc) & !allone(base$exc)) {
        excCI = bootstrap.ci.stu(base$exc,conf.level = conf, R = iter) # Bootstrap over all excess values.
      } else if (allzero(base$exc)) {
        excCI = c(0,0,0)
      } else {
        excCI = c(1,1,1)
      }
...
print_Euler <- function(df,lang,conc) {
  ...
  return( plot(euler(x), quantities = TRUE, lty=1:2, main = conc))
}
...
overlaps.per.item <- function(observations, l, conceptList, overlapFunc = ov2, threshold = 0.1) {
  ...
  # redresult now contains all subjects that are relevant to either of the two concepts mentioned in concept1 and concept2.
  ...
  out = list(stats = stats,tile = stats.tile, data = result, red.data = redresult)
  print(stats)
  invisible(out)
}
...
#' blackOvTile 
#'
#'Prepares an overlap chart based on a dataset of overlaps.
#'
...
blackOvTile <- function(df, statistic = mean, title = "",huelim = c(0,1)){
  return(ggplot(df, aes(x = concept1,y = concept2,fill = {{statistic}})) +
           geom_tile() +scale_fill_gradient2(low="white", high="black",limits = huelim) +
...
#' redundancy
#' 
#' Translates an input of overlap measures into a table of redundancy indexes, with increasing prevalence threshold (= higher sensitivity).
#'
#' @param data Overlap statistics for pairs of concepts. Likely the output of a overlaps.per.item call.
#'
#' @return A tibble listing the concepts and the redundancy intensities for min, q10, q25 and median prevalence thresholds
...
#' @examples
#' 
blueOvTile <- function(df,statistic = mean, title = "",huelim = c(0,1)){
  return(ggplot(df, aes(x = concept1,y = concept2,fill = {{statistic}})) +
           geom_tile(colour="gray90", size=1.5, stat="identity") + 
           geom_text(aes(x = concept1,y = concept2, label = round({{statistic}},2)), color="black", size=rel(4.5)) +
           scale_fill_gradient(low = "white", high = "dodgerblue", space = "Lab", 
                               na.value = "gray90", guide = "colourbar",limits = huelim) +
...
# REPORTING --------------------
#
# Helper functions for preparing reports.
#



reportWilcox <- function(obj, alpha = 0.05, eff = NULL, inMath = FALSE){
  ...
  if (!is.null(eff)) {
    efs = paste0(", r=", round(eff$effsize,2), " (", eff$magnitude, ")")
    efsMath = paste0(", r=", round(eff$effsize,2), "\\,\\mathit{(", eff$magnitude, ")}")
  } else {
    efs = ""
    efsMath = ""
  }
  ...
  if (inMath){
    return(paste0(res,efs))
  } else {
    return(paste0("$",res,efsMath,"$"))
  }
}


reportWilcox0 <- function(obj,alpha = 0.05,inMath = FALSE){
  W = round(obj$statistic,1)
  p = obj$p.value
  if (p <= 0.001) {
    res = paste0("W=",W,", p<0.001")
